...
*/
	...
	// HomoMatrix operator  * (HomoMatrix & a)
	friend  HomoMatrix operator  * (HomoMatrix & a,HomoMatrix & b)
	{
		double temp[4][4]={0};
		int m,n,k;
...
public:
	...
	Chain()
	{
		...
		Lever_Para initpara[6]={{1,200,-90*UNIT,585,0*UNIT, 170*UNIT,-170*UNIT},
								{2,440,  0*UNIT,0,  0*UNIT, 120*UNIT,-80*UNIT},
								{3,115,-90*UNIT,660,0*UNIT, 85*UNIT, -170*UNIT},
...
public:
	...
	static HomoMatrix Forward_Kinematics (double joint_angle[6])
	{
		Chain temp;
	...
	static void Inverse_Kinematics (HomoMatrix position,double theta[6],Chain chain)
	{	
		...
		for (j=0;j<16;++j)
			 {
                   for( i = 0; i< 6;i++)
				   {
					   ...
					   {
						   kout2<<endl<<"第"<<j<<"组解第"<<"第"<<i<<"个角度超限"<<endl;
						   break;
					   }                          
				   }
			 }
...
//	double theta[50][6];
 ...
 void Traceplanning(int index,double qt[6])
	{
		double R=0;
		if(index>=tv)
			R=1;
		...
		HomoMatrix trinterp(double cp)
	{
		...
		HomoMatrix PosInterp(double cp)
	{
...
public:
	...
	double det(double row1[3],double row2[3],double row3[3])
	{
		...
		HomoMatrix PosInterp1(double cp)
	{
		...
		HomoMatrix PosInterp2(double cp)
	{
...
//				int m,x;
		...
		HomoMatrix T0=GSK::Forward_Kinematics (startpoint);
		HomoMatrix T1=GSK::Forward_Kinematics (endpoint);
		T0.H2Trans(flat1);
		T1.H2Trans(flat2);
		double L1=(flat2[0]-flat1[0])*(flat2[0]-flat1[0])+(flat2[1]-flat1[1])*(flat2[1]-flat1[1])+(flat2[2]-flat1[2])*(flat2[2]-flat1[2]);
		int tt=(int)(sqrt(L1/(v*simstep))+0.5);
